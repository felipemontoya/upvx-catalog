(function () {
  ...
  <article class="course" id="<%- course_id %>" role="region" aria-label="<%- name %>">
    <a href="/courses/<%- course_id_encoded %>/about">
  ...
  function loadConfigFromHTML() {
    ...
    const filterAttr = container.getAttribute('data-filter-course-ids');
    ...
    if (filterAttr) {
      try {
        filterCourses = JSON.parse(filterAttr);
      } catch (e) {
        console.error('Error parsing approved courses JSON:', e);
      }
    }

    return { queryAttr, filterCourses };
  }


  // const query = {org: "upv", page_size: 50};
  // const filterCourses = ["course-v1:upv+101+2025_2","course-v1:upv+202+2025_2"];
  const { query, filterCourses } = loadConfigFromHTML();

  const FILTER_COURSE_IDS = filterCourses;
  init(query);
})();
